        ...
        <div class="fresns-editor ms-lg-5">
            ...
            @if (in_array($draft['detail']['state'], [2, 3]))
                <div class="alert alert-warning mt-3" role="alert">
                    <i class="fa-solid fa-triangle-exclamation"></i> {{ $draft['detail']['state'] == 2 ? fs_lang('contentEditReviewTip') : fs_lang('contentEditPublishedTip') }}
                </div>
            @endif
            ...
            <form action="{{ route('fresns.api.post', ['path' => "/api/fresns/v1/editor/{$type}/draft/{$draft['detail']['did']}"]) }}" method="post" id="fresns-editor">
                ...
                <div class="editor-submit d-grid mb-5">
                    <button type="submit" class="btn btn-success btn-lg mt-3 mb-5 mx-3" {{ in_array($draft['detail']['state'], [2, 3]) ? 'disabled' : ''}}>
