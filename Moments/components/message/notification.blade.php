        ...
        <section class="user-secondary d-flex flex-wrap">
            ...
            @if ($notification['content'])
                @if ($notification['contentFsid'])
                    @switch($notification['type'])
                        @case(8)
                            <a href="{{ route('fresns.comment.detail', ['cid' => $notification['contentFsid']]) }}" class="text-decoration-none text-secondary fs-6 mt-2 text-wrap text-break">{{ $notification['content'] }}</a>
                        @break

                        @case(9)
                            <a href="{{ route('fresns.post.detail', ['pid' => $notification['contentFsid']]) }}" class="text-decoration-none text-secondary fs-6 mt-2 text-wrap text-break">{{ $notification['content'] }}</a>
                        @break
